...
subdirectories.map((s) => {
    ...
    fs.readdir(s, function (err, files) {
        ...
        const images = filehound.create().path(s).ext("json").not().findSync();
        const inputCount = images.length;
        let currentCount = 1;

        // read in metadata
        let metaFile = fs.readFileSync(`${s}/metadata.json`);
        const metaContents = JSON.parse(metaFile);

        const filename = metaContents.title.replaceAll(" ", "-").toLowerCase();
